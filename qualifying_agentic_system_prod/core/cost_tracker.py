"""
Cost Tracking System for Qualifying Agentic System
...
class CostTracker:
    """
    Thread-safe cost tracking for Firecrawl and Claude APIs.

    Maintains costs.json file with detailed breakdowns and threshold warnings.
    """

    # API pricing (USD) - Updated December 2024
    PRICING = {
        "firecrawl": {
    ...
    def record_claude_request(
        ...
        with self.lock:
            if model not in self.PRICING["claude"]:
                # Unknown model - estimate with Sonnet pricing
            ...
            # Update Claude totals
            claude = self.costs["breakdown"]["claude"]
            claude["total_requests"] += 1
            claude["total_input_tokens"] += input_tokens
            claude["total_output_tokens"] += output_tokens
            claude["cost_usd"] += total_cost

            # Update per-model breakdown
            if model not in claude["by_model"]:
                claude["by_model"][model] = {
                    "requests": 0,
                    "input_tokens": 0,
                    "output_tokens": 0,
                    "cost_usd": 0.0
                }
            claude["by_model"][model]["requests"] += 1
            claude["by_model"][model]["input_tokens"] += input_tokens
            claude["by_model"][model]["output_tokens"] += output_tokens
            claude["by_model"][model]["cost_usd"] += total_cost
    ...
    def to_summary_string(self) -> str:
        ...
        with self.lock:
            lines = [
                ...
                f"\nClaude:",
                f"  Requests: {self.costs['breakdown']['claude']['total_requests']}",
                f"  Input tokens: {self.costs['breakdown']['claude']['total_input_tokens']:,}",
                f"  Output tokens: {self.costs['breakdown']['claude']['total_output_tokens']:,}",
                f"  Cost: ${self.costs['breakdown']['claude']['cost_usd']:.2f}",
                f"\n{'═'*60}",
