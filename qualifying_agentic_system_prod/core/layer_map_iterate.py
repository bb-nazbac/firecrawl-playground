#!/usr/bin/env python3
"""
L2: Map + Iterative Qualification Layer

This layer is ONLY called when L1 (homepage) was insufficient.
It:
1. Maps the site structure using /v2/map
2. Uses Claude to select relevant pages based on low-confidence questions
...
from core.layer_homepage import (
    call_claude,
...
def build_requalification_prompt(
    ...
    return f"""Re-qualify {domain} for B2B specialty chemical/pharma/materials supplier.
...
Update answers based on new evidence. Classify as: QUALIFIED_TIER_1, QUALIFIED_TIER_2, QUALIFIED_TIER_3, DISQUALIFIED.
- TIER_1: Pharma/engineered materials, specialty products, has inventory
- TIER_2: Chemical products, specialty, B2B
- TIER_3: Mixed/commodity products, B2B
- DISQUALIFY if: not selling products, retail only, commodity only, broker only

JSON response:
{{"sufficient":bool,"company_name":"...","final_classification":"...","disqualification_reason":"COMMODITY_ONLY/NOT_PRODUCT_SELLER/RETAIL_ONLY/BROKER_ONLY/null","answers":{{"sells_products":"YES/NO","is_b2b":"YES/NO","has_inventory_or_manufacturing":"YES/NO","product_type":"SPECIALTY/COMMODITY/MIXED","primary_category":"PHARMA/ENGINEERED_MATERIALS/CHEMICAL/OTHER_TECHNICAL"}},"confidence":{{"sells_products":"HIGH/MEDIUM/LOW","is_b2b":"HIGH/MEDIUM/LOW","has_inventory_or_manufacturing":"HIGH/MEDIUM/LOW","product_type":"HIGH/MEDIUM/LOW","primary_category":"HIGH/MEDIUM/LOW"}},"products_found":["..."],"needs_more_pages":bool,"current_page_summary":"1-2 sentence summary of this page"}}
"""
...
def process_iterative(
    ...
    map_success, site_map, map_error, map_stats = firecrawl_map(
        domain, semaphore=firecrawl_semaphore, limit=100, log_callback=log
    )
    ...
    while pages_scraped_count < max_pages:
        ...
        sel_success, sel_response, sel_in, sel_out, sel_error, sel_stats = call_claude(
            selection_prompt, semaphore=claude_semaphore, model=claude_model, max_tokens=500, log_callback=log
        )
        ...
        scrape_ok, content, scrape_err, s_stats = firecrawl_scrape(
            selected_url, semaphore=firecrawl_semaphore, log_callback=log
        )
        result.credits_used += 1
        ...
        req_success, req_response, req_in, req_out, req_error, req_stats = call_claude(
            requalify_prompt, semaphore=claude_semaphore, model=claude_model, max_tokens=2000, log_callback=log
        )
    ...
    # ─────────────────────────────────────────────────────────
    result.success = True
    result.classification = qualification.get('final_classification') if qualification else None
