#!/usr/bin/env python3
"""
Batch Runner for Qualifying Agentic System
...
from core.domain_queue import DomainQueue, DomainTask, DomainResult
...
class BatchRunner:
    ...
    Features:
    - CSV input support
    - Concurrent Firecrawl and Claude workers
    ...
    def __init__(
        ...
        Args:
            ...
            claude_concurrency: Max concurrent Claude requests
        ...
        self.claude_concurrency = claude_concurrency
    ...
    def qualify_domain(self, task: DomainTask) -> DomainResult:
        ...
        # Estimate Claude costs from tokens
        if result.total_tokens > 0:
            # Rough split: 80% input, 20% output
            input_tokens = int(result.total_tokens * 0.8)
            output_tokens = result.total_tokens - input_tokens
            self.cost_tracker.record_claude_request(
                task.domain, self.claude_model, input_tokens, output_tokens
            )
    ...
    def run(self, csv_path: str) -> List[DomainResult]:
        ...
        """
        print(f"\n{'═'*60}")
        print("QUALIFYING AGENTIC SYSTEM - BATCH RUN")
        print(f"{'═'*60}")
        print(f"Output: {self.output_dir}")
        print(f"Budget: ${self.max_cost_usd}")
        print(f"Concurrency: {self.firecrawl_concurrency} Firecrawl / {self.claude_concurrency} Claude")
        ...
        num_workers = min(self.claude_concurrency, added)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Batch domain qualification')
    ...
    runner = BatchRunner(
        ...
        claude_concurrency=args.concurrency,
