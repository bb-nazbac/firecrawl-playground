...
from core_openai_deep.layer_homepage import process_homepage, HomepageResult, AnalysisSpec
from core_openai_deep.layer_map_iterate import process_iterative, IterativeResult
from core_openai_deep.logger import init_logger
from core_openai_deep.analytics import PipelineAnalytics
...
class PipelineConfig:
    ...
    # Iteration limits - DEEP MODE
    max_pages: int = 20  # Max additional pages to scrape beyond homepage
    min_pages: int = 10  # Minimum pages to scrape before allowing early stop
...
def process_domain(
    ...
    # Only take fast path if homepage is sufficient AND min_pages <= 1
    # If min_pages > 1, we MUST scrape more pages even if homepage seems sufficient
    if l1_result.sufficient and config.min_pages <= 1:
    ...
    if l1_result.sufficient and config.min_pages > 1:
        log(f"\n📍 Homepage claims sufficient, but min_pages={config.min_pages} - forcing L2 iteration...")
    ...
    l2_result = process_iterative(
        ...
        min_pages=config.min_pages,  # Minimum total pages (including homepage)
        openai_model=config.openai_model,
        firecrawl_semaphore=firecrawl_semaphore,
        openai_semaphore=openai_semaphore,
        log_callback=log_callback,
    ...
    def load_csv(self, csv_path: str, limit: int = None) -> List[str]:
        ...
        with open(path, 'r', encoding='utf-8') as f:
            ...
            if has_header:
                ...
                for row in reader:
                    # Support both 'domain' and 'account_domain' column names
                    domain = row.get('domain') or row.get('account_domain') or ''
                    domain = domain.strip()
                    if domain:
...
def main():
    ...
    parser.add_argument('--max-pages', type=int, default=20, help='Max pages per domain (default: 20)')
    parser.add_argument('--min-pages', type=int, default=10, help='Min pages before allowing early stop (default: 10)')
