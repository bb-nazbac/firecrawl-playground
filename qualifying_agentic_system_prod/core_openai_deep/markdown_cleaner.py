...
def strip_markdown(content: str) -> str:
    ...
    content = re.sub(r'!\[([^\]]*)\]\([^)]+\)', lambda m: f'[{m.group(1)}]' if m.group(1) and len(m.group(1)) > 2 else '', content)
    ...
    content = re.sub(r'reCAPTCHA.*?(?:\[Terms\]\([^)]+\)|\n\n)', '', content, flags=re.IGNORECASE | re.DOTALL)
    content = re.sub(r'protected by \*\*reCAPTCHA\*\*.*?Terms\)', '', content, flags=re.IGNORECASE | re.DOTALL)

    # 7. Remove social media icon links
    social_patterns = [
        r'\[(?:Facebook|Twitter|Instagram|Linkedin|Youtube|Tiktok|Pinterest)[^\]]*\]\([^)]+\)\s*',
        r'\[Visit us on (?:Facebook|Twitter|Instagram|X)[^\]]*\]\([^)]+\)\s*',
    ]
    for pattern in social_patterns:
        content = re.sub(pattern, '', content, flags=re.IGNORECASE)

    # 8. Remove app store buttons (iOS/Android download links with images)
    content = re.sub(r'\[!\[Download[^\]]*App\][^\]]*\]\([^)]+\)\s*', '', content, flags=re.IGNORECASE)
