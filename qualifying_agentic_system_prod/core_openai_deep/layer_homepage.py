    ...
    with sem:
        for attempt in range(MAX_RETRIES):
            ...
            try:
                ...
                # reasoning_effort for gpt-5-mini - 'high' causes timeouts, use default (medium)
                # The improved prompts with question guidance should be sufficient
...
def build_homepage_qualification_prompt(domain: str, homepage_content: str, spec: AnalysisSpec) -> str:
    ...
    # Build questions text with guidance (guidance contains critical industry inference rules)
    questions_text = "\n\n".join([
        f"- {q['field']}: {q['question']}\n  GUIDANCE: {q.get('guidance', 'No specific guidance.')}"
...
DO NOT require explicit "Made in China" or "imported from" statements - the industry itself implies imports!
...
- They design/sell their own product line with custom designs
- They have manufacturing partners or overseas suppliers (even if not stated, infer from industry)
...
def process_homepage(
    ...
    # GPT-5-nano is more verbose - needs 8000 tokens
    openai_success, response_text, in_tokens, out_tokens, openai_error, openai_stats = call_openai(
        prompt, semaphore=openai_semaphore, model=openai_model, max_tokens=8000, log_callback=log, analytics=analytics
    )
