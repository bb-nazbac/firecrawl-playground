...
{{
    ...
    "low_confidence_questions": ["question1"],
    "homepage_summary": "1 sentence summary"
}}
...
def process_homepage(
    ...
    if run_waterfall:
        ...
        filter_success, filter_response, f_in, f_out, filter_error, filter_stats = call_openai(
            filter_prompt, semaphore=openai_semaphore, model=openai_model, max_tokens=800, log_callback=log, analytics=analytics
        )
    ...
    # CHEAP: Reduced max_tokens to force concise output
    openai_success, response_text, in_tokens, out_tokens, openai_error, openai_stats = call_openai(
        prompt, semaphore=openai_semaphore, model=openai_model, max_tokens=1500, log_callback=log, analytics=analytics
    )
