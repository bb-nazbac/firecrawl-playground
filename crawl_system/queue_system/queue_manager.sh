#!/bin/bash
#
# Queue Manager - Process pipeline jobs one at a time
...
check_pipeline_success() {
    ...
    local csv_count=$(find "$SCRIPT_DIR/outputs/$client/$domain/" -name "*.csv" 2>/dev/null | wc -l | tr -d ' ')
    [ "$csv_count" -gt 0 ]
}
...
start_pipeline() {
    ...
    CLIENT=$client DOMAIN=$domain "$SCRIPT_DIR/scripts/run_pipeline_robust.sh" "$url" > "$QUEUE_DIR/logs/${client}_${domain}.log" 2>&1 &
    local pid=$!
...
# Main loop
log "========================================="
log "Robust Queue Manager Started"
log "========================================="
...
touch "$QUEUE_FILE" "$COMPLETED_FILE" "$FAILED_FILE"
