...
class SearchConfig:
    ...
    gl: str = "us"  # Country code for geolocation (default: US)
...
class RunConfig:
    ...
    analysis_spec: Optional[str] = None  # No longer required - search-only mode
...
class ConfigLoader:
    ...
    def _validate_and_construct(self, data: Dict[str, Any], config_name: str) -> RunConfig:
        ...
        # analysis_spec is now optional (search-only mode)
        analysis_spec = data.get('analysis_spec')
        if analysis_spec and not isinstance(analysis_spec, str):
            errors.append("Field 'analysis_spec' must be a string if provided")
        ...
        return RunConfig(
            ...
            analysis_spec=data.get('analysis_spec', '').strip() or None,
            test_mode=test_mode,
            resume=resume,
    ...
    def _validate_search_config(self, search_data: Dict[str, Any]) -> tuple:
        ...
        # Validate gl (optional, defaults to "us")
        gl = search_data.get('gl', 'us')
        if not isinstance(gl, str) or not gl.strip():
            errors.append("Field 'search.gl' must be a non-empty string (e.g., 'us', 'es', 'uk')")
        ...
        return SearchConfig(
            ...
            gl=gl.strip().lower()  # Normalize to lowercase
        ), []
