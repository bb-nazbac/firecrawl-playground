#!/usr/bin/env python3
"""
L3 Classification with Client Spec System
Spec-driven LLM classification for flexible client requirements
"""
...
class Logger:
    ...
    def __init__(self, log_path):
        ...
        with open(log_path, 'w', encoding='utf-8') as f:
            f.write("=" * 70 + "\n")
            f.write("SCRIPT: classify_with_spec.py\n")
...
def main():
    parser = argparse.ArgumentParser(description='L3 Classification with Client Spec')
    ...
    log_path = f'../logs/l3_llm_classify/classify_{args.client}_{timestamp}.log'
    logger = Logger(log_path)

    logger.log("=" * 70)
    logger.log("ROUND 06: SPEC-DRIVEN L3 CLASSIFICATION")
    logger.log("=" * 70)
    ...
    l2_pattern = '../outputs/l2_scraped_*.json'
    l2_files = sorted(glob.glob(l2_pattern))

    if not l2_files:
        logger.log(f"❌ ERROR: No L2 files found matching: {l2_pattern}")
        return
